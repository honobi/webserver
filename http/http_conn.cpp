...
//重置一些参数。每次请求--响应都需要重新初始化http_conn，但并不代表重新建立TCP连接，具体看长短连接
void http_conn::init(){
...
http_conn::HTTP_CODE http_conn::parse_content(char* text) {
    ...
    //有消息体，说明是POST请求。POST请求的消息体内容为输入的用户名和密码 比如：user=123&passwd=123
...
//根据url分析出请求的资源是什么，然后将被请求的资源放进共享内存
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    if (if_post == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        int i;
        for (i = 5; http_body[i] != '&'; ++i)
            name[i - 5] = http_body[i];
        name[i - 5] = '\0';

        int j = 0;
        for (i = i + 10; http_body[i] != '\0'; ++i, ++j)
            password[j] = http_body[i];
        password[j] = '\0';
    ...
    if (*(p + 1) == '0') {
        ...
        //为什么要一直申请一块动态内存再释放掉，而不用临时变量：栈区大小是有限的，对于较大的字符串应该放在堆区
        char* m_url_real = (char *)malloc(sizeof(char) * 200);
    ...
    //如果都不是，直接请求root目录下的名字与m_url一致的文件
    else{
        http_priority = 1;
    ...
    //通过stat获取请求资源文件信息，成功则将信息更新到m_file_stat结构体；失败说明资源不存在，返回NO_RESOURCE
    ...
    m_file_address = (char *)mmap(0, m_file_stat.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
     /*     0：由系统分配一个地址。m_file_stat.st_size：内存大小
...
bool http_conn::write() {
    ...
    //若要发送的数据长度为0，表示响应报文为空，那么不需要发数据，重置该http_conn
    if (bytes_to_send == 0) {
        reset_oneshot(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);
        init();
        return true;
    }
    ...
    while (1) {
        ...
        //先写入m_iv[0]状态行、响应头、空行，再写入m_iv[1]的响应体（如果有的话）到m_sockfd
        //writec函数写入的顺序是数组的顺序，也就是m_iv[0]、m_iv[1]...这个顺序
        temp = writev(m_sockfd, m_iv, m_iv_count);
        //返回值：成功：readv所读取的字节数或writev所写入的字节数；错误：返回-1，设置errno
        ...
        if (temp < 0) {
            ...
            //不是缓冲区满，说明有其他错误，释放由mmap创建的共享内存。（write函数返回前需要把本次请求的资源释放掉）
            unmap(); 
            return false;
        }
        ...
        if (bytes_to_send <= 0) {   
            ...
            unmap();//取消mmap映射（write函数返回前需要把本次请求的资源释放掉）
            reset_oneshot(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);//重置该socket可写事件的EPOLLONESHOT

            //如果为长连接，初始化HTTP对象用于处理下一个http报文
...
bool http_conn::process_write(HTTP_CODE ret) {
    ...
    //除FILE_REQUEST有响应体外，其余状态没有响应体，只申请一个iovec，指向写缓冲区
    m_iv[0].iov_base = m_write_buf;
    m_iv[0].iov_len = m_write_idx;
    m_iv_count = 1;
...
void http_conn::process() {
    ...
    if (read_ret == NO_REQUEST) {
        reset_oneshot(m_epollfd, m_sockfd, EPOLLIN, m_TRIGMode);  //重置EPOLLONESHOT
        return;
    }
